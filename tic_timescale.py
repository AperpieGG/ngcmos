...
def trim_target_data_by_time(data):
    ...
    """
    start_threshold = 30 / (24 * 60)  # 30 minutes in days
...
def plot_timescale(times, avg_rms, RMS_model, label):
    fig, ax = plt.subplots(figsize=(6, 8))
    ax.plot(times, avg_rms, 'o', label=f"{label} Data", color='blue')
...
if __name__ == "__main__":
    ...
    plot_timescale(times, avg_rms, RMS_model, label=str(phot_data['TIC_ID']))
