...
def plot_lc(table, gaia_id_to_plot, bin_size=1, exposure_time=10):
    ...
    fluxes_binned = [[np.mean(fluxes[i][j:j + bin_size]) for j in range(0, len(fluxes[i]), bin_size)] for i in range(4)]
    fluxerrs_binned = [
        [np.sqrt(np.sum(fluxerrs[i][j:j + bin_size] ** 2)) / bin_size for j in range(0, len(fluxerrs[i]), bin_size)] for
        i in range(4)]
    sky_binned = [[np.mean(sky[i][j:j + bin_size]) for j in range(0, len(sky[i]), bin_size)] for i in range(4)]
    skyerrs_binned = [
