...
def plot_lc(table, gaia_id_to_plot, bin_size=1, exposure_time=10):
    ...
    fluxes = [gaia_id_data[f'flux_{i}'] for i in range(3, 7)]
    print(gaia_id_data[f'flux_{i}'] for i in range(3, 7))
    fluxerrs = [gaia_id_data[f'fluxerr_{i}'] for i in range(3, 7)]
    sky = [gaia_id_data[f'flux_w_sky_{i}'] - gaia_id_data[f'flux_{i}'] for i in range(3, 7)]
    skyerrs = [np.sqrt(fluxerrs[i] ** 2 + gaia_id_data[f'fluxerr_w_sky_{i}'] ** 2) for i in range(3, 7)]
