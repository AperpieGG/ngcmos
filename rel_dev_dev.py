...
def find_comp_star_rms(comp_fluxes, airmass, comp_mags0):
    ...
    for i in range(Ncomps):
        ...
        airmass_cs = np.polyfit(airmass, comp_flux, 1)  # Fit 1st-order poly to airmass
        airmass_mod = np.polyval(airmass_cs, airmass)   # Evaluate the poly fit
        comp_flux_corrected = comp_flux / airmass_mod   # Airmass corrected flux
        comp_flux_norm = comp_flux_corrected / np.median(comp_flux_corrected)  # Normalize flux
        comp_star_rms_val = np.std(comp_flux_norm)      # Calculate RMS
        if np.isfinite(comp_star_rms_val):
            comp_star_rms = np.append(comp_star_rms, comp_star_rms_val)
        else:
            comp_star_rms = np.append(comp_star_rms, 99.)  # Handle non-finite values
    return comp_star_rms


def find_bad_comp_stars(comp_fluxes, airmass, comp_mags0, sig_level=3., dmag=0.5):
    # Calculate RMS for each comparison star
    comp_star_rms = find_comp_star_rms(comp_fluxes, airmass, comp_mags0)

    # Create a mask for good comparison stars (all stars initially assumed to be good)
    ...
    while True:
        ...
        # Masked magnitudes and RMS values for currently good comparison stars
        comp_mags = np.copy(comp_mags0[comp_star_mask])
        comp_rms = np.copy(comp_star_rms[comp_star_mask])
        N1 = len(comp_mags)  # Number of stars in the current iteration

        # Binning magnitudes and calculating medians for each bin
        edges = np.arange(comp_mags.min(), comp_mags.max() + dmag, dmag)
        dig = np.digitize(comp_mags, edges)
        mag_nodes = (edges[:-1] + edges[1:]) / 2.
        std_medians = np.array([
            np.nan if len(comp_rms[dig == i]) == 0
        ...
        # Interpolate median RMS values using a spline
        spl = Spline(mag_nodes, std_medians)
        mod = spl(comp_mags)  # Modeled RMS for current stars
        mod0 = spl(comp_mags0)  # Modeled RMS for all stars

        # Calculate the standard deviation of the residuals
        std = np.std(comp_rms - mod)

        # Update the mask for good comparison stars based on the sigma clipping level
        comp_star_mask = (comp_star_rms <= mod0 + std * sig_level)
        N2 = np.sum(comp_star_mask)  # Number of stars after filtering

        # Break the loop if the number of good stars remains unchanged
        if N1 == N2:
            break
        elif i > 10.:  # Safety limit for iterations
            break
...
def relative_phot(table, tic_id_to_plot, APERTURE, EXPOSURE):
    ...
    print(f"Comp stars after filtering by RMS and sigma clipping: {len(filtered_tic_ids)}")
