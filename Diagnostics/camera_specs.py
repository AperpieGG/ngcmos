...
"""
This script calculates the pixel scale, horizontal and vertical FOV,
and diagonal FOV of a camera
...
def calculate_pixel_scale(pixel_size, focal_length, sensor_resolution):
    ...
    fov_x_deg = math.degrees(2 * math.atan((sensor_resolution[0] * pixel_size_mm)
                                           / (2 * focal_length)))
    fov_y_deg = math.degrees(2 * math.atan((sensor_resolution[1] * pixel_size_mm)
                                           / (2 * focal_length)))
...
if __name__ == "__main__":
    ...
    parser.add_argument("pixel_size", type=float,
                        help="Pixel size in microns (i.e. 11 for Marana)")
    parser.add_argument("focal_length", type=float,
                        help="Focal length of the camera in mm (i.e. 560 for NGTS)")
    parser.add_argument("sensor_resolution", type=int, nargs=2,
                        help="Sensor resolution in pixels (width height)")
    args = parser.parse_args()

    fov_x, fov_y, fov_diagonal, pixel_scale = (
        calculate_pixel_scale(args.pixel_size, args.focal_length, args.sensor_resolution))
